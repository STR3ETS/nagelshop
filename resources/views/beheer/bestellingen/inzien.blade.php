...
<div class="w-full h-auto">
    <div class="max-w-[1100px] mx-auto py-[1.5rem]">
        ...
        <div class="relative">
            ...
            <h1 class="text-[#191919] text-[38px] font-semibold leading-[1.15] my-2">Bestelling <i class="instrument-serif-font text-[#ff64ba]">inzien</i></h1>
            ...
            <form method="POST" action="{{ route('bestellingen.verzendgegevens', $bestelling) }}" class="w-full space-y-6 flex flex-col items-end">
                ...
                <div class="w-full flex gap-6">
                    <div class="flex flex-col w-1/2">
                        ...
                        <input type="text" name="naam" required value="{{ $bestelling->naam }}" class="w-full px-[0.75rem] py-[0.55rem] ring-1 ring-gray-200 focus:ring-[#ff64ba] outline-none rounded-md">
                    </div>
                    <div class="flex flex-col w-1/2">
                        <label for="email" class="text-sm font-medium">E-mail</label>
                        <input type="email" name="email" required value="{{ $bestelling->email }}" class="w-full px-[0.75rem] py-[0.55rem] ring-1 ring-gray-200 focus:ring-[#ff64ba] outline-none rounded-md">
                    </div>
                </div>
                <div class="w-full flex gap-6">
                    <div class="flex flex-col w-1/3">
                        <label for="adres" class="text-sm font-medium">Adres</label>
                        <input type="text" name="adres" required value="{{ $bestelling->adres }}" class="w-full px-[0.75rem] py-[0.55rem] ring-1 ring-gray-200 focus:ring-[#ff64ba] outline-none rounded-md">
                    </div>
                    <div class="flex flex-col w-1/3">
                        <label for="postcode" class="text-sm font-medium">Postcode</label>
                        <input type="text" name="postcode" required value="{{ $bestelling->postcode }}" class="w-full px-[0.75rem] py-[0.55rem] ring-1 ring-gray-200 focus:ring-[#ff64ba] outline-none rounded-md">
                    </div>
                    <div class="flex flex-col w-1/3">
                        <label for="plaats" class="text-sm font-medium">Plaats</label>
                        <input type="text" name="plaats" required value="{{ $bestelling->plaats }}" class="w-full px-[0.75rem] py-[0.55rem] ring-1 ring-gray-200 focus:ring-[#ff64ba] outline-none rounded-md">
                    </div>
                </div>
                <button type="submit" class="cursor-pointer w-fit px-[1.5rem] py-[0.55rem] bg-[#ff64ba] hover:bg-[#96366c] transition rounded-md text-white text-[15px] font-medium">Opslaan</button>
            </form>
        </div>
        <div class="w-full bg-white p-[1.5rem] rounded-lg mb-4">
            ...
            <ul class="divide-y divide-gray-200 text-[15px] text-[#191919] opacity-90 p-[1.5rem] border-1 border-gray-200 rounded-lg bg-gray-100">
                ...
                @endforeach
                <div class="w-full flex items-center justify-end mt-4 text-[#ff64ba]">
        ...
        <div class="w-full bg-white p-[1.5rem] rounded-lg">
            ...
            <form method="POST" action="{{ route('bestellingen.tracktrace', $bestelling) }}" class="w-full flex gap-4 items-end">
                ...
                <div class="flex flex-col w-full">
                    ...
                    <input type="text" name="track_trace" required value="{{ $bestelling->track_trace }}" class="w-full px-4 py-2 ring-1 ring-gray-200 rounded-md focus:ring-[#ff64ba] outline-none">
                </div>
                <button type="submit" class="cursor-pointer w-fit px-[1.5rem] py-[0.55rem] bg-[#ff64ba] hover:bg-[#96366c] transition rounded-md text-white text-[15px] font-medium">Opslaan</button>
            </form>
        </div>
